...
| **CERT-05** | User downloads their certificate (PKCS#12) via `POST .../pkcs12` by providing the correct PIN. | The server returns a binary file (status 200). The file is encrypted with the provided PIN.         | **Critical**  | Integration   |
| **CERT-05-PUB** | User downloads their public certificate (CRT) via `GET .../download`. | The server returns a binary CRT/PEM file (status 200). | High | Integration |
| **CERT-06** | User attempts to download PKCS#12 by providing an incorrect PIN.               | The server returns a 400 (Bad Request) error with a message about the incorrect PIN.               | **Critical**  | Integration   |
| **CERT-07** | User attempts to download another user's certificate (PKCS#12 or Public).                                | The server returns a 403 (Forbidden) or 404 (Not Found) error.                                       | **Critical**  | Integration   |
