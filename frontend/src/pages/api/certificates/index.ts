...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const fetchPendingCertificates = async () =>
      fetchPendingCertificatesFromBackend(backendUrl, token, userId);
    ...
    console.info(
      `Getting certificates: ${backendUrl}/users/${userId}/certificates/list${queryString}`,
    );
    ...
    if (!statusFilter || statusFilter === "ALL") {
      ...
      if (!pendingResult.error) {
        backendData.total =
          (backendData.total || 0) + pendingCertificates.length;
      }
    }
